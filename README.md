...
- **调度窗口**：盘内(9:30-11:30/13:00-15:00)自动执行当日决策；工作日 12:01-12:59 与
  17:30-18:01 每 1 分钟轮询拉取决策；读到**次日**决策不立即下单，留待该  交易日盘中自动实单。
- **文件命名**：决策/成交/持仓文件**全带时间戳** （日内多版本互不覆盖）；
  读取取当日最新一份，消费后归档到 ； 为本机防重账本（固定名，跨批累计）。
...
# 生成的 results/holdings 每次落盘固定名 + <名>_YYYYMMDD_HHMMSS 时间戳副本（日内多版本历史）
```
