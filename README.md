...
- **调度窗口**：盘内自动执行当日决策； 09:00-10:00（早盘，覆盖盘前投递）、12:58-13:00（午间）、17:30-18:01（尾盘）每 1 分钟轮询同步决策；窗口外计算式睡眠（步长≤1h，每小时一醒保心跳/终端检测，不空转轮询），心跳带 sleeping_until 供看板显示睡眠态。
...
标准库 HTML 看板，两页签；治理类操作一律 **X-Dash-Token 鉴权 + 二次确认 + 告警审计留痕**。

**股票交易页签**
- 账户卡：现金四要素 + **持仓汇总**（总市值/总成本/浮盈合计/只数/前重仓摘要）；持仓表按市值降序，成本 vs 现价 vs 市值 vs 浮盈同列对照
  （注：当日盈亏需昨收/昨结基准，broker 快照未提供，暂缺）
- 当日待执行 decisions（含版本/归档计数）· 执行回报 results
- 回报行「链」按钮：展开 **决策→委托→成交** 链路（决策源因/volume + 终端委托成交状态；手动单/历史单明示无匹配）
- 今日委托：**手动/自动统一视图**——每行标 `source`（manual=手动、decision=LKL 自动单），
  筛选下拉（全部/手动/自动）+ 手动·自动计数；终端离线时回退 manual_orders 回捞快照并横幅提示，
  **空数据与故障态明确区分**（离线横幅 ≠ 「暂无委托」）
- 对账四源一致 · 告警中心（CRIT/WARN 汇总 + 明细）

**系统运行状态页签**
- 顶部横幅：模式/终端/凭证/绑定账户状态 + **距开市/距下次拉取/距唤醒倒计时**（秒级，距开市 ≤5 分钟转红；闭市睡眠中显示唤醒时刻）
- 运行前自检（doctor）：一键跑 python/gmtrade/凭据/终端/账户/交换目录/决策样例 7 项，等价 CLI `lkl doctor`
- 安全治理：切实盘/演练/紧急停止/解除（二次确认+原因，审计写告警中心 CRIT/INFO）；
  **风控上限可视化**——单笔/当日下单/当日操作只数三输入框保存即写 `.secrets/gm.env` 即时生效（0=不限），无需改文件
- 交换目录文件健康（存在/今日/损坏/for_date）

**开市提醒（服务端）**：看板进程常驻时，距下一开市 ≤5 分钟自动推一条 WARN——
配置 `GM_ALERT_WEBHOOK` 后可达钉钉/企微/Server酱 等，不依赖人盯屏。
